...
int main(void) {
    ...
    printf("n\tavg_0_fixp\tmean_0_fixp\tavg_1_fixp\tmean_1_fixp\n");
    ...
    // printf("one_fixp_counter: %zu\n", one_fixp_counter);

    // assert(factorial(4) == 24);

    // double null_fixp_mean = ((double)null_fixp_counter / SAMPLES_NUM) * factorial(N);

    // double one_fixp_mean = ((double)one_fixp_counter / SAMPLES_NUM) * factorial(N);

    // printf("Srednia liczba permutacji BEZ stalych punktów: %f\n", null_fixp_mean);

    // printf("Srednia liczba permutacji z JEDNYM stalym punktem: %f\n", one_fixp_mean);
